...
const debug = require('debug')('loopback:juggler:observer');
...
ObserverMixin.observe = function(operation, listener) {
  debug('observe: %s', operation);
  this._observers = this._observers || {};
  this._observersLock = this._observersLock || {};
  if (!this._observers[operation]) {
    this._observers[operation] = [];
    this._observersLock[operation] = 0;
  }
...
ObserverMixin.removeObserver = function(operation, listener) {
  debug('removeObserver: %s', operation);
  if (!(this._observers && this._observers[operation])) return;

  const index = this._observers[operation].indexOf(listener);
  if (index !== -1) {
    if (this._observersLock[operation]) {
      const observer = this._observers[operation][index];
      this._observers[operation][index] = null;
      return [observer];
    } else {
...
ObserverMixin.clearObservers = function(operation) {
  debug('clearObservers: %s', operation);
  if (!(this._observers && this._observers[operation])) return;

  if (this._observersLock[operation]) {
    for (let i = 0; i < this._observers.length; ++i) {
      this._observers[operation][i] = null;
    }
  } else {
...
ObserverMixin.notifyObserversOf = function(operation, context, callback) {
  ...
  debug('notifyObserversOf: %s', operation);
  ...
  this._notifyBaseObservers(operation, context, function doNotify(err) {
    ...
    async.eachSeries(
      ...
      function(err) {
        --self._observersLock[operation];
        if (self._observersLock[operation] === 0) {
          debug('cleaning up observers for %s', operation);
          debug('Before clean: %d', self._observers[operation].length);
          self._observers[operation] = self._observers[operation].filter(
            function(observer) { return observer !== null; }
          );
          debug('After clean: %d', self._observers[operation].length);
        }
...
ObserverMixin.notifyObserversAround = function(operation, context, fn, callback) {
  debug('notifyObserversAround: %s', operation);
  const self = this;
  context = context || {};
